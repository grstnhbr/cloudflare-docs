...
- **Traffic logs**: HTTP and network traffic proxied through the endpoint appears in [Gateway logs](/cloudflare-one/insights/logs/dashboard-logs/gateway-logs/), with the specific proxy endpoint indicated.
...
All connections proxied through Cloudflare Gateway have a maximum guaranteed duration of 10 hours. For more information, refer to [Troubleshooting](/cloudflare-one/access-controls/troubleshooting/#long-lived-ssh-sessions-disconnect).
